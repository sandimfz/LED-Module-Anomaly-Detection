...
def detect_uniform_content(
    ...
    # Brightness variation threshold: std < 25 and less than 20% of mean.
    # Raised from (std < 30, ratio < 0.25) to reduce false positives on
    # simple-but-normal content (solid color ad, gradient backgrounds).
    # A truly frozen/stuck display has very low variation: std < 20-25.
    std_ratio = std_brightness / mean_brightness if mean_brightness > 0 else 1.0

    if std_brightness < 25 and std_ratio < 0.20:
        severity = min(1.0 - (std_brightness / 30.0), 1.0)
